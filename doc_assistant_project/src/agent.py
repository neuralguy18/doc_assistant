...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    prompt = prompt_template.format(
        ...
        conversation_history ="\n".join([m.content for m in history]) if history else ""
    )


    intent_result: UserIntent = intent_llm.invoke(prompt)
    ...
    if intent_result.intent_type == "qa":
        ...
        next_step = "qa_agent"  # fallback default
...
def summarization_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    # Load the prompt template dedicated to summarization
    prompt_template = get_chat_prompt_template("summarization")

    # Construct full message list (same pattern as qa_agent)
    messages = prompt_template.invoke({
        "input": state["user_input"],
        "chat_history": state.get("messages", [])
    }).to_messages()

    # Invoke the React agent with SummarizationResponse schema
...
def calculation_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    # Load the prompt template for calculations
    prompt_template = get_chat_prompt_template("calculation")

    # Construct messages (same pattern as qa_agent)
    messages = prompt_template.invoke({
        "input": state["user_input"],
        "chat_history": state.get("messages", [])
    }).to_messages()

    # Invoke the React agent with CalculationResponse schema
    result, tools_used = invoke_react_agent(
        CalculationResponse,
        messages,
        llm,
