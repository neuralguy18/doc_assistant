...
def create_calculator_tool(logger: ToolLogger):
    """
    Creates a safe calculator tool that evaluates mathematical expressions.
    Only allows basic arithmetic and logs all tool usage.
    """
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A string representing a math expression (e.g., "5 + 3 * 2")

        Returns:
            A formatted string containing the result or an error message.
        """
        ...
        try:
            ...
            if not SAFE_PATTERN.match(expression):
                error_msg = (
                    "Invalid expression. Only numbers and basic math operators "
